...
let main = 
    ...
    | 8 -> 
      (
      match (Sys.argv.(1)) with
      | "-step" -> if (int_of_string (Sys.argv.(7))) == 0 then print_string("yes") else print_string("no") (* fire_act (int_of_string (Sys.argv.(2))) (Sys.argv.(3)) (float_of_string (Sys.argv.(4))) (Sys.argv.(5)) (Sys.argv.(6)) (int_of_string (Sys.argv.(7))) *)
      | _ -> print_string ("Wrong input!\nUnrecognized or misused option: " ^ (Sys.argv.(1)) ^ "\n")
      )
